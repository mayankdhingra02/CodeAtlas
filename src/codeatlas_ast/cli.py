...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        prog="codeatlas-ast",
        description="AST-only Python repository indexing and retrieval",
    )
    ...
    try:
        if args.command == "index":
            ...
            print(
                ...
                if args.json
                else _mapping("AST index created", payload)
            )
        ...
        if args.command == "stats":
            ...
            print(
                json.dumps(payload, indent=2, sort_keys=True)
                if args.json
                else _mapping("AST index statistics", payload)
            )
            return 0
...
def _mapping(title: str, payload: Mapping[str, object]) -> str:
    width = max(map(len, payload), default=0)
    return "\n".join(
        [title, *(f"{key:<{width}} : {value}" for key, value in payload.items())]
    )
...
def _render(payload: Mapping[str, object]) -> str:
    ...
    if not isinstance(snippets, (list, tuple)):
        raise ValueError("retrieval payload 'snippets' must be a list or tuple")

    for number, raw_snippet in enumerate(snippets, start=1):
        if not isinstance(raw_snippet, Mapping):
            raise ValueError("every retrieval snippet must be an object")
        snippet: Mapping[str, Any] = raw_snippet
        lines += [
            f"\n[{number}] {snippet['file']}:{snippet['line_start']}-{snippet['line_end']}",
            (
                f"    {snippet['qualified_name']} [{snippet['kind']}] "
                f"score={float(snippet['score']):.2f}"
            ),
            f"    {snippet['reason']}",
            "-" * 72,
            str(snippet["code"]),
